      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-x-hidden">
        ...
        <main className="p-4 lg:p-8 space-y-6 max-w-7xl w-full mx-auto">
          ...
          <div className="space-y-1">
            <h1 className="text-3xl font-extrabold tracking-tight text-slate-900">Ajukan Reimbursement</h1>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-0 text-xs font-semibold px-2 relative">
              ...
              <div className="flex items-center gap-3 z-10">
                ...
                <div>
                  <h4 className="font-bold text-slate-800 leading-tight">Upload struk</h4>
                  <p className="text-[10px] text-slate-400 font-medium">Foto atau gambar</p>
                </div>
              </div>

              <div className={`hidden md:block flex-1 h-0.5 mx-4 transition-colors duration-300 ${
                currentState === 'upload' ? 'bg-slate-200' : 'bg-emerald-600'
              }`} />

              <div className="flex items-center gap-3 z-10">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold shrink-0 transition-all duration-300 ${
                  currentState === 'upload' ? 'bg-slate-100 text-slate-400 border border-slate-200' :
                ...
                <div>
                  <h4 className={`font-bold leading-tight ${currentState === 'upload' ? 'text-slate-400' : 'text-slate-800'}`}>AI OCR</h4>
                  <p className="text-[10px] text-slate-400 font-medium">Otomatis baca data</p>
                </div>
              </div>

              <div className={`hidden md:block flex-1 h-0.5 mx-4 transition-colors duration-300 ${
                currentState === 'upload' || currentState === 'ocr' ? 'bg-slate-200' : 'bg-emerald-600'
              }`} />
              ...
              <div className="flex items-center gap-3 z-10">
                ...
                <div>
                  <h4 className={`font-bold leading-tight ${currentState === 'review' || currentState === 'success' ? 'text-slate-800' : 'text-slate-400'}`}>Review & kirim</h4>
                  <p className="text-[10px] text-slate-400 font-medium">Verifikasi data</p>
                </div>
              </div>

              <div className={`hidden md:block flex-1 h-0.5 mx-4 transition-colors duration-300 ${
                currentState === 'success' ? 'bg-emerald-600' : 'bg-slate-200'
              }`} />
              ...
              <div className="flex items-center gap-3 z-10">
                ...
                <div>
                  <h4 className={`font-bold leading-tight ${currentState === 'success' ? 'text-slate-800' : 'text-slate-400'}`}>Terkirim</h4>
                  <p className="text-[10px] text-slate-400 font-medium">Menunggu approval</p>
                </div>
              </div>
          ...
          {currentState === 'upload' && (
            <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
              <div className="md:col-span-6 space-y-4">
                <h2 className="text-lg font-bold text-slate-900">Upload foto struk atau nota</h2>
                ...
                <div className="flex items-center gap-2.5 pt-2">
                  ...
                  <button onClick={handleUploadAction} className="inline-flex items-center gap-1.5 px-4 py-2.5 bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 text-xs font-bold rounded-xl transition shadow-sm">
                    <Upload size={14} /> Unggah File
                  </button>
                </div>
              </div>
              
              <div onClick={handleUploadAction} className="md:col-span-6 border-2 border-dashed border-slate-200 hover:border-emerald-500 bg-slate-50/50 rounded-2xl p-8 flex flex-col items-center justify-center gap-2 text-center cursor-pointer transition group">
                <div className="w-9 h-9 bg-white border border-slate-100 rounded-xl flex items-center justify-center text-slate-400 group-hover:text-emerald-600 shadow-sm transition">
                  <Upload size={16} />
                </div>
                <span className="text-xs font-bold text-slate-700">Klik atau drop struk di sini</span>
                <span className="text-[10px] text-slate-400">JPG, PNG, HEIC, PDF maks. 10MB</span>
              </div>
            </div>
          )}
          ...
          {currentState === 'ocr' && (
            <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
              {/* Sisi Kiri Mockup Gambar Struk */}
              <div className="md:col-span-4 bg-slate-100 rounded-xl overflow-hidden shadow-sm relative group aspect-3/4 max-w-60 mx-auto md:mx-0 flex items-center justify-center">
              ...
              <div className="md:col-span-8 space-y-5">
                ...
                <div className="space-y-1">
                  <h2 className="text-xl font-extrabold text-slate-900">Membaca strukmu..</h2>
                ...
                <div className="pt-4">
                  <button onClick={handleOcrFinished} className="px-4 py-2 bg-slate-900 text-white font-bold text-xs rounded-xl hover:bg-slate-800 transition">
          ...
          {currentState === 'review' && (
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm lg:col-span-8 space-y-5">
                <div className="space-y-1">
                  <div className="inline-block px-2.5 py-0.5 bg-[#DDF2E8] text-[#198754] text-[10px] font-bold rounded-md">
                    Data berhasil diekstrak
                  </div>
                  <h2 className="text-xl font-extrabold text-slate-900">Review & lengkapi data</h2>
                  <p className="text-xs text-slate-400">
                    Periksa data yang dibaca AI. Field yang ditandai dapat kamu ubah jika ada yang tidak sesuai.
                  </p>
                </div>

                <form className="space-y-4 text-xs font-bold text-slate-700" onSubmit={(e) => { e.preventDefault(); handleFormSubmit(); }}>
                  
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-1.5 text-left">
                      <label className="text-slate-500 font-bold">Proyek</label>
                      <div className="relative">
                        <select 
                          value={proyek} 
                          onChange={(e) => setProyek(e.target.value)}
                          className="w-full bg-white border border-slate-200 rounded-xl pl-3 pr-10 py-3 font-medium text-slate-800 appearance-none focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-[#008F5D] transition-all"
                        >
                    ...
                    <div className="space-y-1.5 text-left">
                      <label className="text-slate-500 font-bold">Pos anggaran</label>
                      <div className="relative">
                        <select 
                          value={posAnggaran}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-1.5 text-left">
                      <div className="flex items-center gap-2">
                        <label className="text-slate-500 font-bold">Merchant</label>
                      ...
                      <input 
                        ...
                        className="w-full bg-white border border-slate-200 rounded-xl px-3 py-3 font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-[#008F5D] transition-all" 
                      />
                    </div>
                    <div className="space-y-1.5 text-left">
                      <div className="flex items-center gap-2">
                        <label className="text-slate-500 font-bold">Tanggal Transaksi</label>
                        <span className="bg-[#E0F2FE] text-[#0369A1] font-bold text-[9px] px-1.5 py-0.5 rounded-md leading-none shadow-sm select-none">dari OCR</span>
                      </div>
                      <div className="relative">
                        <input 
                          ...
                          className="w-full bg-white border border-slate-200 rounded-xl px-3 py-3 font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-[#008F5D] transition-all [color-scheme:light]" 
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-1.5 text-left">
                      <div className="flex items-center gap-2">
                        <label className="text-slate-500 font-bold">Nominal (IDR)</label>
                        <span className="bg-[#E0F2FE] text-[#0369A1] font-bold text-[9px] px-1.5 py-0.5 rounded-md leading-none shadow-sm select-none">dari OCR</span>
                      </div>
                      <div className="relative">
                        <span className="absolute left-3.5 top-3.5 font-medium text-slate-400 select-none">Rp</span>
                        <input 
                          type="text" 
                          value={nominal} 
                          onChange={(e) => setNominal(e.target.value)}
                          className="w-full bg-white border border-slate-200 rounded-xl pl-9 pr-3 py-3 font-medium text-slate-800 font-mono focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-[#008F5D] transition-all" 
                        />
                      </div>
                    </div>
                    <div className="space-y-1.5 text-left">
                      <label className="text-slate-500 font-bold">Kategori Bukti</label>
                      <div className="relative">
                        <select 
                          value={kategoriBukti}
                  ...
                  <div className="space-y-1.5 text-left">
                    <label className="text-slate-500 font-bold">Keterangan</label>
                    <textarea 
                      rows={3} 
                      value={keterangan} 
                      onChange={(e) => setKeterangan(e.target.value)}
                      className="w-full bg-white border border-slate-200 rounded-xl px-3 py-3 font-medium text-slate-800 resize-none focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-[#008F5D] transition-all" 
                    />
                  </div>

                  <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                    <button type="button" onClick={handleResetForm} className="inline-flex items-center gap-1 text-xs font-bold text-slate-500 hover:text-slate-800 transition">
                      <ArrowLeft size={14} /> Upload ulang
                    </button>
                    <div className="flex items-center gap-2">
                      <button type="button" onClick={handleFormSubmit} className="px-4 py-2.5 bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 text-xs font-bold rounded-xl transition">
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm lg:col-span-4 space-y-3">
                <div>
                  <h3 className="font-extrabold text-sm text-slate-900">Bukti Struk</h3>
                  <p className="text-[10px] text-slate-400 font-medium">Hasil scan.</p>
                </div>
                <div className="w-full rounded-xl overflow-hidden bg-slate-50 border border-slate-200 aspect-3/4 relative flex items-center justify-center shadow-inner">
                  <img 
                    src="/bukti_struk.png" 
                    alt="Bukti Struk" 
                    className="w-full h-full object-cover"
          ...
          {currentState === 'success' && (
            <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm max-w-2xl mx-auto text-center space-y-6">
              ...
              <div className="space-y-2 max-w-md mx-auto">
                <h2 className="text-xl font-extrabold text-slate-900">Pengajuan terkirim!</h2>
                <p className="text-xs text-slate-500 leading-relaxed font-medium">
                  Pengajuan <span className="font-mono font-bold text-slate-700">RB-2026-0143</span> senilai <span className="font-bold text-slate-900">Rp {nominal}</span> telah diteruskan ke <span className="font-bold text-slate-800">Muhammad Alvin Ababil</span> (Project Manager) untuk validasi. Kamu akan mendapat notifikasi saat statusnya berubah.
                </p>
              </div>

              <div className="flex items-center justify-center gap-3 pt-2">
                <button onClick={handleResetForm} className="px-4 py-2.5 bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 text-xs font-bold rounded-xl transition shadow-sm">
